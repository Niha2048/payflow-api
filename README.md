...
In both `UserService` and `TransactionService`, the respective repository is injected using `@Autowired`.

### Explanation
- The `@Service` annotation marks each service class as a Spring bean.
- The `@Autowired` annotation tells Spring Boot to inject the correct repository bean into the service.
- **Comment added in the code:**  
  *“Spring Boot scans for @Repository beans and injects them here automatically. At startup, it creates a proxy implementation of the repository interface and wires it into this field.”*
- At startup, Spring scans the `repository` package, finds `UserRepository` and `TransactionRepository`, creates proxy implementations, and wires them into the service classes automatically.
- This means developers don’t need to manually instantiate repositories — Spring handles dependency injection behind the scenes.
